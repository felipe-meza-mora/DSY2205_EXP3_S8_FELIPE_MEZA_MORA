 ...
 /**
  ...
  loadUsers(): void {
    this.usersService.getUsers().subscribe(
      (response: any) => { // Cambia el tipo a `any` temporalmente para verificar
        if (Array.isArray(response)) {
          this.users = response; // Si es un array, se asigna directamente
        } else {
          this.users = response.users; // Ajusta esto si la estructura es `{ users: [...] }`
        }
      },
      (error) => {
