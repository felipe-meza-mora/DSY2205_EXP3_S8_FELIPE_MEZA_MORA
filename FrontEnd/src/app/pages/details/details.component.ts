...
export class DetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (productId) {
      const numericProductId = Number(productId);  // Convierte el id a número
      this.productService.getProductById(numericProductId)  // Pasa el id numérico
        .subscribe(response => {
          console.log(response);  // Verifica la respuesta
          this.product = response;  // Asignar directamente si response es el producto
        });
    }
  }
